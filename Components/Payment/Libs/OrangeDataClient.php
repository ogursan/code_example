...
namespace Shopen\AppBundle\Components\Payment\Libs;
 ...
 * @version 2.0.1
 *
 * Присутствует здесь по причине того, что разработчики OrangeData не осилили PSR и Composer.
 * Источник: https://github.com/orangedata-official/PHP-OrangeData-official
 */
class OrangeDataClient {
    ...
    public function create_order($id, $type, $customerContact, $taxationSystem, $group = null, $key = null) {
        ...
        /**
         * Здесь враги без предупреждения поменяли нам интерфейс API
         */
        $this->order_request->key = $key ?: $this->inn;
    ...
    public function add_user_attribute($name, $value) {
        if (strlen($name) > 0 && strlen($name) < 65 && strlen($value) > 0 && strlen($value) < 176) {
    ...
    public function send_order() {
        $jsonstring = json_encode($this->order_request, 1024);
        $sign = $this->sign_order_request($jsonstring);
        is_numeric($this->url) ? $curl =  $this->prepare_curl($this->edit_url($this->url, TRUE)) : $curl =  $this->prepare_curl($this->url . '/api/v2/documents/');
        $headers = array(
    ...
    public function get_order_status($id) {
        ...
        $curl = is_int($this->url) ? $this->prepare_curl($this->edit_url($this->url, TRUE) . $this->inn . '/status/' . $id) : $this->prepare_curl($this->url . '/api/v2/documents/' . $this->inn . '/status/' . $id);
    ...
    public function create_correction($id, $correctionType, $type, $description, DateTime $causeDocumentDate, $causeDocumentNumber, $totalSum, $cashSum, $eCashSum, $prepaymentSum, $postpaymentSum, $otherPaymentTypeSum, $tax1Sum, $tax2Sum, $tax3Sum, $tax4Sum, $tax5Sum, $tax6Sum, $taxationSystem, $group = null, $key = null) {
        ...
        if (is_numeric($correctionType) && preg_match('/^[01]$/', $correctionType)) {
    ...
    public function post_correction() {
        $jsonstring = json_encode($this->correction_request, 1024);
        if(!$jsonstring){
            throw  new \Exception('JSON encode error:' . json_last_error_msg());
        }
        $sign = $this->sign_order_request($jsonstring);
        $curl = is_numeric($this->url) ? $this->prepare_curl($this->edit_url($this->url, FALSE)) : $this->prepare_curl($this->url . '/api/v2/corrections/');
        $headers = array(
            "Content-Length: " . strlen($jsonstring),
            "Content-Type: application/json; charset=utf-8",
            "X-Signature: " . $sign
    ...
    public function get_correction_status($id) {
        ...
        $curl = is_numeric($this->url) ? $this->prepare_curl($this->edit_url($this->url,FALSE) . $this->inn . '/status/' . $id) : $this->prepare_curl($this->url . '/api/v2/corrections/' . $this->inn . '/status/' . $id);
        curl_setopt($curl, CURLOPT_POST, false);
        $answer = curl_exec($curl);
        $info = curl_getinfo($curl);
        switch ($info['http_code']) {
